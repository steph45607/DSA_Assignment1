...
void arrayListType<elemType>::print() const
{
    ...
    for (int i = 0; i < length; i++){
        cout << i+1 << ". " << list[i].getName() << " " << list[i].getPhone() << " " << list[i].getEmail() << endl;
    }
    cout << "---------------------------" << endl;
}
...
template <class elemType>
int arrayListType<elemType>::seqSearch(string item) const
{
    int loc;
    bool found = false;
    for (loc = 0; loc < length; loc++)
        if ((string)list[loc].getName() == (string)item)
        {
            found = true;
            break;
        }
